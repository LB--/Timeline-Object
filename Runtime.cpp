...
inline void Move(LPRDATA rdPtr, signed long& I)
{
	if(rdPtr->TimeDir > 0)
	{
		++I;
	}
	else if(rdPtr->TimeDir < 0)
	{
		--I;
	}
}
...
short WINAPI DLLExport CreateRunObject(LPRDATA rdPtr, LPEDATA edPtr, fpcob cobPtr)
{
	...
	*/
	...
	rdPtr->Time = edPtr->StartTime;
	rdPtr->TimeDir = edPtr->StartTimeDir;
	rdPtr->Timeline = new map<unsigned long, vector<Event> >;
	rdPtr->Trigger = edPtr->TriggerDef;
	rdPtr->TimeTrigger = edPtr->TriggerChange;
	rdPtr->ErrorStr = "";
...
*/
	...
	if(rdPtr->Timeline != NULL) delete rdPtr->Timeline;
	rdPtr->Timeline = NULL;
...
*/
	...
	if(rdPtr->TimeDir != 0) //Only do this when time is moving
	{
		//Don't want to skip anything
		for(signed long i = 0; i != rdPtr->TimeDir && rdPtr->TimeDir != 0; Move(rdPtr, i))
		{
			if(rdPtr->TimeTrigger)
			{
				rdPtr->rRd->GenerateEvent(6);
			}
			if(rdPtr->Timeline->find(rdPtr->Time) != rdPtr->Timeline->end()) //Only trigger events if needed
			{
				for(unsigned long l = 0; l < (*rdPtr->Timeline)[rdPtr->Time].size(); l++) //Trigger each event
				{
					rdPtr->ToTrigger = (*rdPtr->Timeline)[rdPtr->Time][l].Name;
					rdPtr->TriggerID = l;
			...
			if(rdPtr->TimeDir < 0)
			{
				...
				else if(rdPtr->Time == 0) //MUST be else if!
				{
					rdPtr->TimeDir = 0;
				}
			}
