...
PROPS_DATA_START()
	...
	PropData_EditNumber(PROPID_STARTIME,PSTR("Start Time"),PSTR("Position in the timeline to start at")),
	PropData_EditNumber(PROPID_TIMEDIR,PSTR("Time Speed"),PSTR("Starting speed for time (Can be negative)")),
	PropData_CheckBox(PROPID_EVENTRIGGER,PSTR("Trigger Events by Default"),PSTR("Triggers Events by default when Time is moving")),
	PropData_CheckBox(PROPID_CHANGETRIGGER,PSTR("Allow triggering of \"On Position Change\""),PSTR("Triggers the \"On Position Change\" condition when time changes. Disable for efficiency!")),
...
//	}
	switch(nPropID)
	{
	case PROPID_STARTIME:
		return new CPropDWordValue(edPtr->StartTime);
		break;
	case PROPID_TIMEDIR:
		return new CPropDWordValue(edPtr->StartTimeDir);
		break;
	}
#endif // !RUN_ONLY
	return NULL;
...
//	}
	...
	switch(nPropID)
	{
	case PROPID_EVENTRIGGER:
		return(edPtr->TriggerDef);
		break;
	case PROPID_CHANGETRIGGER:
		return(edPtr->TriggerChange);
		break;
	}
...
//	}
	...
	{
	case PROPID_STARTIME:
		edPtr->StartTime = ((CPropDWordValue*)pValue)->m_dwValue;
		break;
	case PROPID_TIMEDIR:
		edPtr->StartTimeDir = ((CPropDWordValue*)pValue)->m_dwValue;
		break;
	}
...
//	}
	...
	{
	case PROPID_EVENTRIGGER:
		edPtr->TriggerDef = nCheck;
		mvRefreshProp(mV, edPtr, PROPID_EVENTRIGGER, TRUE);
		break;
	case PROPID_CHANGETRIGGER:
		edPtr->TriggerChange = nCheck;
		mvRefreshProp(mV, edPtr, PROPID_CHANGETRIGGER, TRUE);
		break;
	}

							#pragma warning( default: 4800 ) //warning C4800: forcing value to bool 'true' or 'false' (performance warning)
...
//	edPtr->swidth = 32;
//	edPtr->sheight = 32;
	edPtr->TriggerDef = true;
	edPtr->TriggerChange = false;
	edPtr->StartTime = 0;
	edPtr->StartTimeDir = 0;
